...
The service is `START_NOT_STICKY`. Process death ends live SSH transports. Each connection setup worker is explicitly owned and cancellation closes SSHJ resources, interrupts prompt waits, and finishes credential cleanup before a retry starts. Automatic reconnect uses a bounded per-host attempt and backoff policy, requires reusable credentials, pauses without a usable network, and creates a new shell; tmux or screen is required for remote process continuity. Exhausted retries remain available for explicit reauthentication instead of silently discarding the session.
