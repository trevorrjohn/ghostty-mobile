...
| Cancellation, retry, keepalive, and typed failures | `Partial` | `Partial` | Android has per-host bounded retry policy, reauthentication, keepalive, abrupt-EOF recovery, off-main transport shutdown, and failure classification but still needs prompt cancellation of every in-progress transport stage. iOS has typed failures, parent-channel loss detection, close-first teardown, and manual retry with a fresh transport; true keepalive and prompt in-progress connect cancellation require additional Citadel support, and network-aware retry policy remains. |
