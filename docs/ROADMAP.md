...
| Trusted-host inspection and removal | `Partial` | `Partial` | Android lists full saved fingerprints, serializes trust updates, and guards removal while sessions are active. iOS permits removal only outside an active attempt but still needs a complete management surface. Destination normalization remains on both platforms. |
