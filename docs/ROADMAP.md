...
| Cancellation, retry, keepalive, and typed failures | `Partial` | `Partial` | Android has retry, reauthentication, keepalive, abrupt-EOF recovery, off-main transport shutdown, and failure classification but still needs complete cancellation and user-configurable policy. iOS has typed failures, parent-channel loss detection, close-first teardown, and manual retry with a fresh transport; true keepalive and prompt in-progress connect cancellation require additional Citadel support, and network-aware retry policy remains. |
