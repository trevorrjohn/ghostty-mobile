...
| Keyboard-interactive, OTP, and MFA challenges | `Implemented` | `Blocked` | Citadel 0.12.1 and its pinned NIOSSH fork do not expose RFC 4256 methods, wire messages, or challenge callbacks. iOS requires an upstream or reviewed vendored transport implementation; OTP input must not be approximated through the PTY. |
...
| Bounded per-host startup command | `Implemented` | `Implemented` | Both validate one optional 1,024-byte single-line command and write it once to each new interactive PTY before user input is enabled. It repeats on honest reconnect and never runs for SFTP. |
| Per-host environment and initial directory | `Planned` | `Planned` | Deferred until connection setup has typed, auditable configuration. |
...
| Scrollback navigation | `Implemented` | `Implemented` | iOS retains 10,000 lines, supports fractional touch scrolling with reduced-motion-aware inertia, indirect pointer-wheel and trackpad scrolling, accessibility scrolling, immutable viewport state, and a Live return; physical pointer-device validation remains. |
...
| Per-host remote-notification policy | `Implemented` | `Implemented` | Both parse bounded requests and enforce persisted ask/allow/block policy. iOS permits one attempt-scoped approval, rejects stale approval after teardown, and requests system notification authorization only when delivery is allowed. |
| Bell, progress, hyperlinks, title, and working directory | `Implemented` | `Implemented` | iOS drains bounded Ghostty effects, presents typed progress and secondary remote metadata without replacing trusted host identity, schedules bell feedback, routes ordered PTY responses, and preserves explicit copy/open actions for bounded `http` and `https` OSC 8 hyperlinks. Physical notification and effect validation remains. |
...
| Tablet, landscape, split-screen, and external-keyboard workflows | `Partial` | `Partial` | Android keeps terminal rows and header controls cutout-safe over a continuous full-screen background, retains multi-window and desktop caption insets, and exposes a session info/actions modal from a fixed first keyboard-row control with a floating fallback when the row is hidden; both platforms still need their full device and interaction matrices. |
