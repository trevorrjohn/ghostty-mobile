...
class SshKeyStore(private val context: Context) {
    ...
    fun inspect(privateKey: ByteArray): SshKeyDetails = inspectSshPrivateKey(privateKey, names())
...
internal fun inspectSshPrivateKey(privateKey: ByteArray, existingNames: Collection<String> = emptyList()): SshKeyDetails {
    ...
    return SshKeyDetails(uniqueName(baseName.take(80), existingNames), encrypted)
}
...
private fun parseOpenSsh(text: String): OpenSshDetails? = runCatching {
    ...
    val fingerprint = Base64.getEncoder().withoutPadding()
        .encodeToString(MessageDigest.getInstance("SHA-256").digest(publicKey)).take(12)
